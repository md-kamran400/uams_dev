...
export default function AssetRecords({ assetId }: { assetId: string }) {
  ...
  useEffect(() => {
    setIsLoading(true);
    api.assetDetails.listSubmissions(assetId)
  ...
  const exportData = (format: 'csv' | 'excel') => {
    const headers = ['Date', 'Shift', 'Operator', 'Status', 'Fields Recorded', 'Rejection Reason'];
    const rows = filteredSubmissions.map(s => [
      fmtDate(s.date), s.shift, s.operatorName || 'Unknown', s.status, Object.keys(s.values ?? {}).length, s.rejectionReason || ''
    ]);
    if (format === 'csv') {
          ...
          {isLoading ? (
            ...
            <>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h3 className="text-sm font-bold text-gray-800 mb-1">Logbook Records</h3>
                  <p className="text-xs text-gray-500">{submissions.length} form submissions for this asset</p>
                </div>
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-800">
                              Shift {sub.shift} — {fmtDate(sub.date)}
                            </p>
