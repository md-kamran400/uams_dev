...
export default function DGInventory({ utilityTypeId }: { utilityTypeId: string }) {
  ...
  const exportCsv = () => {
    const headers = ['Item', 'Part No.', 'Asset', 'Qty in Stock', 'Reorder Level', 'Unit Cost', 'Status'];
    const rows = filteredItems.map(i => [
      i.name, i.partCode, i.assetId || 'Global', `${i.currentQty} ${i.unit}`, `${i.minStock} ${i.unit}`, String(i.unitCost), getStatus(i)
    ]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editTarget) {
        ...
        const created = await api.spares.create({ name: form.name!, partCode: form.partCode!, ...form, utilityTypeId });
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500">Track stock levels and reorder thresholds across this utility type.</p>
        </div>
        ...
        <table className="min-w-[700px] w-full border-collapse">
          <thead>
            <tr className="border-b-2 border-gray-200">
              {['Item', 'Part No.', 'Asset', 'Qty in Stock', 'Reorder Level', 'Unit Cost (₹)', 'Status', 'Actions'].map(h => (
              ...
              return (
                <motion.tr key={item.id} variants={rowVariants} initial="hidden" animate="visible" className="border-b border-gray-100 hover:bg-blue-50 transition-colors">
                  ...
                  <td className="py-3 px-4 text-xs text-gray-500">
                    {item.assetId ? 'Linked to Asset' : 'Global / Unassigned'}
                  </td>
